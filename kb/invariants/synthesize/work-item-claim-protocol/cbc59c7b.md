...
confidence: 0.95
sources: 0
entities: [AnswerPipelineWorkItem, NextPipelineWorkItem, SetPipelineWorkItemResponse, Pipeline.ContinueSessionForItem, Strategy.Decode, Strategy.Apply, normalizeFactPath, pipeline_work_items]
refs: ['src://knomit/internal/store/pipeline_index.go@6356eab8', 'src://knomit/internal/synthesize/pipeline.go@6356eab8', 'src://knomit/internal/synthesize/review_claim_test.go@6356eab8', kb/architecture/store/pipeline-session-phase-cas/30657f2b.md, kb/decisions/synthesize/one-engine-two-drivers/19c69b3f.md]
---
...
3. CLAIM-AND-MARK in one CAS: `UPDATE pipeline_work_items SET response = ? WHERE id = ? AND response IS NULL` (store.AnswerPipelineWorkItem). rows == 0 means another caller already answered it: a BENIGN NO-OP, not an error, matching AdvancePipelineSessionPhase's convention.
4. APPLY the mutations, only on a won claim.

WHY: distill and merge mint a FRESH UUID filename per apply (normalizeFactPath), so applying twice writes duplicate synthesized facts rather than converging. Before this protocol, review applied then marked (duplicates on resubmit or on a crash between the two) while hypothesize marked then applied, and NextPipelineWorkItem was a bare SELECT with SetPipelineWorkItemResponse a blind UPDATE, so two racing continues both peeked and both applied.
